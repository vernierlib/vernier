...
//
// Modified by Guillaume J. Laurent on january 10th, 2018.
...
namespace Eigen {
    ...
    namespace internal {
        ...
        template <> struct type_matio<MAT_T_INT8> {
            typedef int8_t type;
        };

        template <> struct type_matio<MAT_T_UINT8> {
            typedef uint8_t type;
        };

        template <> struct type_matio<MAT_T_INT16> {
            typedef int16_t type;
        };

        template <> struct type_matio<MAT_T_UINT16> {
            typedef uint16_t type;
        };

        template <> struct type_matio<MAT_T_INT32> {
            typedef int32_t type;
        };

        template <> struct type_matio<MAT_T_UINT32> {
            typedef uint32_t type;
        };

        template <> struct type_matio<MAT_T_SINGLE> {
            typedef float type;
        };

        template <> struct type_matio<MAT_T_DOUBLE> {
            typedef double type;
        };

        template <> struct type_matio<MAT_T_INT64> {
            typedef int64_t type;
        };

        template <> struct type_matio<MAT_T_UINT64> {
            typedef uint64_t type;
        };

        template <> struct class_matio<MAT_C_DOUBLE> {
            typedef double type;
        };
    ...
    public:
        ...
        explicit operator bool() const {
            return _file != NULL && _errstr.str().size() == 0;
        }
        ...
        int open(const char * filename,
            ...
            if (NULL == _file) {
                ...
                throw std::runtime_error(_errstr.str());
            }
            _mode = mode;
            _ft = fileversion;
            return 0;
